...
-- Homepage: http://_json.luaforge.net/
-- Version: 0.9
...
local function _decode_scanArray(s,startPos)
  ...
  repeat
    ...
    assert(startPos<=stringLen, 'JSON String ended unexpectedly scanning array.')
    object, startPos = _json.decode(s,startPos)
...
local function _decode_scanObject(s,startPos)
  ...
  repeat
    ...
    -- Scan the key
    key, startPos = _json.decode(s,startPos)
    ...
    startPos = _decode_scanWhitespace(s,startPos+1)
    assert(startPos<=stringLen, 'JSON string ended unexpectedly searching for value of key ' .. key)
    value, startPos = _json.decode(s,startPos)
    object[key]=value
    ...
    for i,j in pairs(v) do
      if (not (bArray)) or (i~='n') then
        if _isEncodable(i) and _isEncodable(j) then
          ...
          if bArray then
            if (i~='n') then
			  table.insert(rval, _json.encode(j))
            end
          else
            table.insert(rval, '"')
